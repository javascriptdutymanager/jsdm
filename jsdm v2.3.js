...
let startBotV2 = setInterval(function() {
    ...
    let ownerList = ["Имя Фамилия ", "Имя"];
    // После Имени и Фамилии добавьте пробел.
    // Владелец (Доступ ко всем командам)
    let adminList = [];
    // Администраторы
    let moderList = [];
    // Модераторы
    ...
    // Помощники
    let prefix = [".к", "!к", ".л", "!л"];
    let prefixAll = [".к", " !к", " .л", " !л"];
    let idCommands = [`id`, `ид`, `айди`];
    let downLoadBot = ["гит", "git"];
    // префикс
                ...
                if (messageTxt === `${prefix[prefixid]} шабы` && lastFocusName === owner[adm]) {
                    setTimeout(function() {
                        ...
                        for (let tpCount = 0; tpCount < templatesList.length; tpCount++) {
                            ...
                            //console.log(templatesList[tpCount].name.slice(7));
                            if (tempPush.length >= 10) {
                                input.innerHTML = ` 📝 Ваши последние 10 шаблонов: <br>
                                                   ...
                                                   ${templatesList[4].name.slice(7)}<br><br>
        ...
        for (let adm = 0; adm < owner.length; adm++) {
            for (let prefixid = 0; prefixid < prefix.length; prefixid++) {
                if (messageTxt === `${prefix[prefixid]} префиксы` && lastFocusName === owner[adm]) {
                    setTimeout(function() {
                        input.innerHTML = `📃 Список ваших префиксов: <br> ${prefixAll}<br>
